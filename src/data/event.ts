// Commented out: TopEvent.tsx and ExploreEvents.tsx now fetch real events from
// the Event API (eventService.getPublicEvents()) instead of this static mock list.
/*
import concert from "../assets/images/landingPage/Subtract.svg";
import concert3 from "../assets/images/landingPage/Subtract (1).svg";
import concert2 from "../assets/images/landingPage/Subtract (2).svg";
import concert4 from "../assets/images/landingPage/Subtract (3).svg";
import concert5 from "../assets/images/landingPage/Subtract (4).svg";
import concert6 from "../assets/images/landingPage/Subtract (5).svg";
import type { EventType } from "../types/eventType";

export const topEvents: EventType[] = [
  {
    id: "1",
    tag: "concert",
    image: concert,
    date: "28 May 2026",
    time: "3PM - 5PM",
    title: "Midnight Pulse",
    location: "Victoria Island Rooftop Lounge",
    price: "₦49,999.98",
  },
  {
    id: "2",
    tag: "concert",
    image: concert2,
    date: "28 May 2026",
    time: "3PM - 5PM",
    title: "Vibe City Live",
    location: "Victoria Island Rooftop Lounge",
    price: "₦49,999.98",
  },

  {
    id: "3",
    tag: "concert",
    image: concert3,
    date: "28 May 2026",
    time: "3PM - 5PM",
    title: "The Social Escape",
    location: "Victoria Island Rooftop Lounge",
    price: "₦49,999.98",
  },
  {
    id: "4",
    tag: "concert",
    image: concert4,
    date: "28 May 2026",
    time: "3PM - 5PM",
    title: "Midnight Pulse",
    location: "Victoria Island Rooftop Lounge",
    price: "₦49,999.98",
  },
  {
    id: "5",
    tag: "concert",
    image: concert5,
    date: "28 May 2026",
    time: "3PM - 5PM",
    title: "Vibe City Live",
    location: "Victoria Island Rooftop Lounge",
    price: "₦49,999.98",
  },
  {
    id: "6",
    tag: "concert",
    image: concert6,
    date: "28 May 2026",
    time: "3PM - 5PM",
    title: "The Social Escape",
    location: "Victoria Island Rooftop Lounge",
    price: "₦49,999.98",
  },
];
*/
